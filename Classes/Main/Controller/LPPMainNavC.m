...
@interface LPPMainNavC () <UIGestureRecognizerDelegate>

@end

@implementation LPPMainNavC

+ (void)initialize {
    
    if (self == [LPPMainNavC class]) {
        // 设置导航栏
        // 系统适配
        UINavigationBar *navBar = nil;
        if (CurrentSystemVersion < 9.0) {
            // 9.0 前支持的方法
            navBar = [UINavigationBar appearanceWhenContainedIn:[LPPMainNavC class], nil];
        } else {
            // 9.0 后支持的方法
            navBar = [UINavigationBar appearanceWhenContainedInInstancesOfClasses:@[[LPPMainNavC class]]];
        }
...
//        navBar.barTintColor = DIYColor(225, 55, 39);
        
        navBar.barTintColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"writeOrder_bg"]];
